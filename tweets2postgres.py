...
def parse_tweets_columns_activity_streams(tweet):
    id_str = tweetutils.extract_tweet_id(tweet.get('id', ''))
    posted_time = tweetutils.from_postedTime(tweet['postedTime'])
    rt = tweetutils.parse_retweet(tweet)
    actor_id = tweetutils.extract_user_id(tweet.get('actor', {}).get('id', ''))
    ...
    return {'id': id_str,
            ...
            'actor_id': actor_id, 
            'body': tweet.get('body', ''),
            'generator': act.get('generator', {}).get('displayName', ''),
...
def parse_tweets_columns_native(tweet):
    ...
    dt = datetime.datetime.strptime(created_at, tweetutils.GNIP_DATETIME_FORMAT)
    user = tweet.get('user', {})
    source_name, source_url = tweetutils.extract_source(tweet.get('source', ''))
...
def parse_users_columns_native(tweet):
    created_at = tweet.get('created_at')
    obs_time = datetime.datetime.strptime(created_at, tweetutils.GNIP_DATETIME_FORMAT)
    user = tweet.get('user')
    created_at = user.get('created_at')
    created_time = datetime.datetime.strptime(created_at, tweetutils.GNIP_DATETIME_FORMAT)
...
def parse_users_columns_activity_streams(tweet):
    obs_time = tweetutils.from_postedTime(tweet['postedTime'])

    user = tweet.get('actor')
    created_time = tweetutils.from_postedTime(user['postedTime'])
    id_str = tweetutils.extract_user_id(user['id'])
...
if __name__=="__main__":
    ...
    print 'Insert\tValueError\tnottweet\tLast tweet timestamp'
